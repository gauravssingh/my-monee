...
format_reset_time() {
    ...
    # Check if gdate (GNU date) is available
    if command -v gdate >/dev/null 2>&1; then
        local epoch
        epoch=$(gdate -d "$reset_iso" +%s 2>/dev/null)
        if [ -n "$epoch" ]; then
            if [ "$style" = "time" ]; then
                gdate -d "@$epoch" +"%H:%M" 2>/dev/null
            else
                gdate -d "@$epoch" +"%b %-d, %H:%M" 2>/dev/null | tr '[:upper:]' '[:lower:]'
            fi
            return
        fi
    fi

    # Fallback to Python for cross-platform macOS/Linux ISO date parsing
    python3 -c "
import sys, datetime
try:
    iso = '$reset_iso'
    ...
    if '$style' == 'time':
        ...
        print(dt.strftime('%b %-d, %H:%M').lower())
except Exception:
    pass
" 2>/dev/null
}
...
# ── Extract JSON data ────────────────────────────────────
model_name=$(echo "$input" | jq -r '.model.display_name // .model // "agy"')
...
model_id=$(echo "$input" | jq -r '.model.id // .model // ""' | tr '[:upper:]' '[:lower:]')
